<x-filament-panels::page>
    <div class="space-y-6">
        ...
        <div class="bg-gray-800 rounded-xl shadow p-6">
            <h3 class="text-lg font-semibold text-white mb-1">Step 1: Upload WordPress SQL Dump</h3>
            <p class="text-sm text-gray-400 mb-4">
                Upload the <code class="text-xs bg-gray-700 px-1.5 py-0.5 rounded">wedgietu_wp_nnfpq.sql</code> file exported from phpMyAdmin.
            </p>
            ...
            <div class="bg-gray-800 rounded-xl shadow p-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                    <div class="bg-gray-700/50 rounded-lg p-4 text-center">
                        ...
                        <p class="text-xs text-gray-400 mt-1">Video Posts</p>
                    </div>
                    <div class="bg-gray-700/50 rounded-lg p-4 text-center">
                        <p class="text-2xl font-bold text-gray-300">{{ number_format($parseStats['postmeta_entries'] ?? 0) }}</p>
                        <p class="text-xs text-gray-400 mt-1">Meta Entries</p>
                    </div>
                    <div class="bg-gray-700/50 rounded-lg p-4 text-center">
                        <p class="text-2xl font-bold text-gray-300">{{ $parseStats['terms'] ?? 0 }}</p>
                        <p class="text-xs text-gray-400 mt-1">Tags/Terms</p>
                    </div>
                    <div class="bg-gray-700/50 rounded-lg p-4 text-center">
                        <p class="text-2xl font-bold text-gray-300">{{ $parseStats['term_relationships'] ?? 0 }}</p>
                        <p class="text-xs text-gray-400 mt-1">Relationships</p>
                    </div>
                </div>

                {{-- Preview Table --}}
                @if(!empty($previewVideos))
                    <h4 class="text-sm font-medium text-gray-300 mb-2">Preview (first 10 videos):</h4>
                        ...
                        <table class="w-full text-sm">
                            <thead class="bg-gray-700/50">
                                <tr>
                                    ...
                                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-400">Bunny ID</th>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-400">Duration</th>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-400">Views</th>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-400">Category</th>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-400">Tags</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-700">
                                @foreach($previewVideos as $video)
                                    <tr class="hover:bg-gray-700/30">
                                        <td class="px-3 py-2 text-gray-400 font-mono text-xs">{{ $video['wp_id'] }}</td>
            ...
            <div class="bg-gray-800 rounded-xl shadow p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <div>
                        ...
                        <select wire:model="importUserId" class="w-full rounded-lg border-gray-600 bg-gray-700 text-white text-sm" required>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-300 mb-1">Delay Between Batches</label>
                        <select wire:model="delayMs" class="w-full rounded-lg border-gray-600 bg-gray-700 text-white text-sm">
        ...
        @if($isImporting)
            <div class="bg-gray-800 rounded-xl shadow p-6">
                <h3 class="text-lg font-semibold text-white mb-4">Importing...</h3>
        ...
        @if($importComplete)
            <div class="bg-gray-800 rounded-xl shadow p-6">
                <div class="text-center mb-6">
                    ...
                    <h3 class="text-xl font-bold text-white">Import Complete!</h3>
            ...
            <div class="bg-gray-800 rounded-xl shadow p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-400">Upload the WordPress database dump exported from phpMyAdmin</p>
                        </div>
                    </div>
                    <div class="flex items-start gap-3">
                        <div class="w-8 h-8 rounded-full bg-primary-900/30 flex items-center justify-center flex-shrink-0">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-400">Parser extracts vidmov_video posts, Bunny Stream embeds, tags, categories, and view counts</p>
                        </div>
                    </div>
                    <div class="flex items-start gap-3">
                        <div class="w-8 h-8 rounded-full bg-primary-900/30 flex items-center justify-center flex-shrink-0">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-400">Videos are imported into HubTube's embedded_videos table with all metadata preserved</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-4 p-3 bg-blue-900/20 rounded-lg border border-blue-800">
                    <p class="text-xs text-blue-400">
                        ...
                        <strong>Tailored for WedgieTube:</strong> This importer is specifically built to parse the <code class="bg-blue-800 px-1 rounded">MKdOzH8c_</code> prefixed tables,
                        extract <code class="bg-blue-800 px-1 rounded">vidmov_video</code> posts, and map Bunny Stream Library <code class="bg-blue-800 px-1 rounded">250371</code> embed codes.
                    </p>
                </div>
            </div>
        @endif
