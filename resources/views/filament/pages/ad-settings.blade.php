<x-filament-panels::page>
    ...
    <div class="mt-10">
        <div class="flex items-center justify-between mb-4">
            <div>
                <h2 class="text-xl font-bold dark:text-white">Video Ad Creatives</h2>
        ...
        {{-- Ad Form Modal --}}
        @if($showAdForm)
        <div class="mb-6 p-6 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" wire:model="adFormData.name" class="fi-input block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white" placeholder="e.g. Summer Sale Pre-Roll" />
                    @error('adFormData.name') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label class="block text-sm font-medium mb-1 dark:text-gray-300">Placement *</label>
                    <select wire:model="adFormData.placement" class="fi-input block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    <label class="block text-sm font-medium mb-1 dark:text-gray-300">Weight / Priority</label>
                    <input type="number" wire:model="adFormData.weight" min="1" max="100" class="fi-input block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white" />
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div class="max-h-40 overflow-y-auto border rounded-lg p-2 dark:border-gray-600">
                        @foreach($this->categories as $catId => $catName)
                            <label class="flex items-center gap-2 py-1 cursor-pointer">
                                <input type="checkbox" wire:model="adFormData.category_ids" value="{{ $catId }}" class="rounded border-gray-300 dark:border-gray-600" />
                                <span class="text-sm dark:text-gray-300">{{ $catName }}</span>
                            </label>
                        @endforeach
                ...
                <div>
                    <label class="block text-sm font-medium mb-1 dark:text-gray-300">Target User Roles</label>
                    ...
                    <div class="border rounded-lg p-2 dark:border-gray-600">
                        @foreach(['guest' => 'Guests (not logged in)', 'default' => 'Default Users (free)', 'pro' => 'Pro Users', 'admin' => 'Admins'] as $role => $label)
                            <label class="flex items-center gap-2 py-1 cursor-pointer">
                                <input type="checkbox" wire:model="adFormData.target_roles" value="{{ $role }}" class="rounded border-gray-300 dark:border-gray-600" />
                                <span class="text-sm dark:text-gray-300">{{ $label }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
            ...
            <div class="flex items-center gap-2 mb-4">
                <input type="checkbox" wire:model="adFormData.is_active" id="ad_active" class="rounded border-gray-300 dark:border-gray-600" />
                <label for="ad_active" class="text-sm dark:text-gray-300">Active</label>
            </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 dark:bg-gray-800">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left font-medium text-gray-600 dark:text-gray-300">Placement</th>
                        <th class="px-4 py-3 text-center font-medium text-gray-600 dark:text-gray-300">Weight</th>
                        <th class="px-4 py-3 text-left font-medium text-gray-600 dark:text-gray-300">Targeting</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($this->videoAds as $ad)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 {{ !$ad->is_active ? 'opacity-50' : '' }}">
                            <td class="px-4 py-3 dark:text-white font-medium">{{ $ad->name }}</td>
                            <td class="px-4 py-3">
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium
                                    {{ $ad->type === 'mp4' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300' : '' }}
                                    {{ $ad->type === 'vast' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900 dark:text-purple-300' : '' }}
                                    {{ $ad->type === 'vpaid' ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-300' : '' }}
                                    {{ $ad->type === 'html' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900 dark:text-amber-300' : '' }}
                                ">
                                    {{ strtoupper($ad->type) }}
                                </span>
                            </td>
                            <td class="px-4 py-3">
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium
                                    {{ $ad->placement === 'pre_roll' ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' : '' }}
                                    {{ $ad->placement === 'mid_roll' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-300' : '' }}
                                    {{ $ad->placement === 'post_roll' ? 'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300' : '' }}
                                ">
                                    {{ str_replace('_', '-', $ad->placement) }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-center dark:text-gray-300">{{ $ad->weight }}</td>
                            ...
                            <td class="px-4 py-3 text-center">
                                <button wire:click="toggleAdActive({{ $ad->id }})" class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium cursor-pointer
                                    {{ $ad->is_active ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' : 'bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400' }}
                                ">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <button wire:click="openAdForm({{ $ad->id }})" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 text-xs font-medium">
                                        Edit
                                    </button>
                                    <button wire:click="deleteAd({{ $ad->id }})" wire:confirm="Delete this ad creative?" class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 text-xs font-medium">
                                        Delete
                                    </button>
                                </div>
                            </td>
    ...
    <div class="mt-8 p-4 rounded-lg bg-gray-100 dark:bg-gray-800">
        <h3 class="text-lg font-semibold mb-4 dark:text-white">Ad Guidelines</h3>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            <div>
                <h4 class="font-medium mb-2 dark:text-gray-200">Video Roll Ads</h4>
            ...
            <div>
                <h4 class="font-medium mb-2 dark:text-gray-200">Display Ads (Grid/Sidebar)</h4>
                <ul class="list-disc list-inside text-gray-600 dark:text-gray-400 space-y-1">
            ...
            <div>
                <h4 class="font-medium mb-2 dark:text-gray-200">Targeting Tips</h4>
                <ul class="list-disc list-inside text-gray-600 dark:text-gray-400 space-y-1">
