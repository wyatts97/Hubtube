...
// Category and tag edits need full video edit rights; privacy and delete don't.
const page = usePage();
const canEditMetadata = computed(() => page.props.auth?.user?.can_edit_video ?? false);
...
<template>
    ...
    <div v-if="selected.length" class="card p-3 mb-4 flex flex-wrap items-center gap-2">
        ...
        <select v-model="bulkAction" class="input w-auto" :aria-label="t('studio.bulk_action')">
            ...
            <option v-if="canEditMetadata" value="category">{{ t('studio.bulk_category') }}</option>
            <option v-if="canEditMetadata" value="tags">{{ t('studio.bulk_tags') }}</option>
