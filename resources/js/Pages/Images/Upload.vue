...
const handleDrop = (e) => {
    ...
    if (file && acceptedTypes.includes(file.type)) {
        ...
        uploadError.value = 'Please drop a valid image file (JPG, PNG, GIF, WebP, BMP).';
    }
};
...
const handleFile = (file) => {
    ...
    if (file.size > 50 * 1024 * 1024) {
        uploadError.value = 'File size must be under 50MB.';
        return;
    }
...
const fileSizeFormatted = computed(() => {
    ...
    const bytes = form.image_file.size;
