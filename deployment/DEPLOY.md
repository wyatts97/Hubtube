...
**The three things it can reclaim**, safest first:

1. **Duplicate HLS copies** — no re-encoding at all. The `processed/hls` tree is renamed aside and the master playlist rewritten, after which `hls_playlist_url` returns null and the player falls back to progressive MP4. Every rendition still exists, so `qualities_available` is untouched. This is the largest and cheapest win; do it first. `storage:repackage-hls {video}` rebuilds the tree from the MP4s (a remux — minutes, not hours) if you change your mind after accepting. **It refuses to run while HLS is the only copy of any advertised quality**, which is the one way this feature could make a video unplayable.
2. **Rendition re-encode** — redoes one `processed/{quality}.mp4` at the reclaim preset. The old file is *hardlinked* aside, so it costs nothing and the canonical path keeps serving the old bytes for the entire encode. No re-approval, no points, no notification: it goes through the same `updateRenditions()` path as "encode missing renditions".
3. **Original re-compression** — usually the biggest single file. The output goes to a **new filename** (`{stem}_r{id}.mp4`), which sidesteps the 30-day mp4 cache and means `video_path` still points at the old file until you accept.

Every result is verified before it is offered: the same length to within 250 ms, a real video stream, at least 10 KB, and a saving over the threshold. Anything else is discarded, the old file restored, and the row marked skipped with the reason. **`storage:reclaim --target=hls --limit=5`** is the intended way in — run it small, compare the review page's before/after against `du -sh` on the directory, then widen. The Media Library's "Reclaim space…" button does the same thing for a selection, resolving 200 selected HLS segments to one reclaim and spelling out every refusal first.

One setting interacts: reclaiming HLS from a video while **`generate_hls` is still on** means the next encode of that video writes the copy straight back. The confirm modal says so and offers to switch it off in the same click.

**Also fixed here: the Pro download served the wrong file.** `download()` sorted the quality labels as strings, which orders them `original, 720p, 480p, 360p, 1080p` — so a 1080p+720p ladder handed out **720p**, and the `original` branch never broke out of the loop, so a video with no rendition on disk served the raw upload. It now picks the highest rendition numerically with the original as a genuine fallback (`Video::bestDownloadPath()`). Worth knowing because the old fallback preferred the largest file on the box, which is exactly what this feature exists to shrink.
